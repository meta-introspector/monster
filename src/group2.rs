...
fn main() {
    ...
    println!("{}", "=".repeat(70));
    println!("MONSTER WALK GROUP 2: After 8080");
    println!("{}", "=".repeat(70));
    println!("\nFull Monster order: {}", monster_order);
    
    // The remaining digits after "8080"
    let full_str = monster_order.to_string();
    let after_8080 = &full_str[4..]; // Skip "8080"
    
    println!("Digits after '8080': {}", after_8080);
    println!("Length: {} digits\n", after_8080.len());
    
    // Search for best matches starting from different digit positions
